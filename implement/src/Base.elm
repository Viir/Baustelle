...
dictMapValues valueMap dict =
    dict |> Dict.map (\_ value -> valueMap value)
...
pickRandomItemFromListWeighted randomSeed listWeighted =
    let
        listWithBound =
            listWeighted
                |> List.foldl
                    (\( item, itemWeight ) listSoFar ->
                        let
                            lastBound =
                                listSoFar
                                    |> List.head
                                    |> Maybe.andThen (\( _, lastItemBound ) -> Just lastItemBound)
                                    |> Maybe.withDefault 0
                        in
        ...
        ( weightUnitIndex, _ ) =
            Random.step (Random.int 0 totalBound) randomSeed
    in
    listWithBound
        |> List.filter (\( _, itemBound ) -> itemBound < weightUnitIndex)
        |> List.head
        |> Maybe.andThen (\( item, _ ) -> Just item)
