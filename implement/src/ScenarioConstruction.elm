...
jointIdsBase : Scenario.State -> JointId
jointIdsBase scenario =
    ([ scenario.joints |> Dict.keys, scenario.permSupport |> Dict.keys ] |> List.concat |> List.maximum |> Maybe.withDefault 0) + 1


withPermSupportAddedAtLocations : List ( Float2, Bool ) -> Scenario.State -> Scenario.State
withPermSupportAddedAtLocations supportLocations scenario =
    let
        ...
        permSupportAddition =
            permSupportAdditionWithOutsetFlag |> dictMapValues Tuple.first

        outsetAddtion =
            permSupportAdditionWithOutsetFlag |> Dict.filter (\_ ( _, isOutset ) -> isOutset) |> Dict.keys |> Set.fromList
    in
    { scenario
...
withJointsAddedAtLocations : List Float2 -> Scenario.State -> Scenario.State
withJointsAddedAtLocations locations scenario =
    let
        ...
        jointsAddition =
            locations |> List.indexedMap (\i location -> ( i + jointIdBase, { location = location, velocity = ( 0, 0 ) } )) |> Dict.fromList
    in
    { scenario | joints = scenario.joints |> Dict.union jointsAddition }


withBeamsAddedForMaxLength : Float -> Scenario.State -> Scenario.State
withBeamsAddedForMaxLength maxLength scenarioBeforePropagationOfSupport =
    let
        ...
        allJointsDictIdAndLocation =
            [ scenario.permSupport, scenario.tempSupport, scenario.joints |> dictMapValues (\joint -> joint.location) ]

        allJointsIds =
            allJointsDictIdAndLocation |> List.map Dict.keys |> List.concat |> Set.fromList
                            ...
                            |> List.filterMap
                                (\destJointId ->
                                    case Scenario.distanceFromJointsInScenario scenario ( origJointId, destJointId ) of
                                        ...
                                        Just distance ->
                                            if origJointId < destJointId then
                                                Just ( ( origJointId, destJointId ), { builtLength = distance } )
