module Console exposing (MouseButtonType(..), MouseEvent, MouseEventType(..), eventMouseMoveAttribute, setMouseEventAttribute, setMouseEventAttributeWithOffsetMapped)
...
setMouseEventAttribute =
    setMouseEventAttributeWithOffsetMapped identity
            ...
            (\event ->
                on event
                    (mouseEventDecoder
                        ...
                        |> Decode.map (Maybe.andThen (\event1 -> Just (mouseEventOffsetMapped offsetMap event1)))
                    )
            )
